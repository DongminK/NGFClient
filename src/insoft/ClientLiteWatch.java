	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 4) {
			...
			try {
				...
				String[] commands = { "GET_CONFIG", "GET_CHANNEL", "SET_WATCH" };
						...
						if ("list".compareTo(command) == 0)
							...
							if (handler == null)
								...
								try {
									...
									msgResponse = conn.send(requestMsg);
								...
								if (command.equals("GET_CONFIG")) {
									try {
										...
										FileWriter fw = null;
										try {
											fw = new FileWriter("WatchFail.txt", true);
	...
	private static void fileLoad(ArrayList<Message> ltServerData) {
		...
		try {
			...
			while ((tmp = br.readLine()) != null) {
				...
				msg.setString("watch_name", st.nextToken().trim());
				msg.setString("watch_type", st.nextToken().trim());
